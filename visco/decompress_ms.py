...
def construct_main_ds(zarr_path: str, column: str, batch_size:int):
    ...
    for baseline_idx, baseline in enumerate(baselines):
        ...
        for corr_name in correlations:
            ...
            all_baseline_tasks.append((vis_reconstructed, row_indices, corr_name, corr_indices, nrows))
    ...
    for batch_start in range(0, len(all_baseline_tasks), batch_size):
        ...
        for idx, (_, row_indices, corr_name, corr_indices, nrows) in enumerate(batch_tasks):
...
def open_dataset(zarr_path: str, column: str = 'COMPRESSED_DATA', group: str = None,batch_size:int=50):
    """"
    ...
    """
    if group is None:
        maintable = construct_main_ds(zarr_path=zarr_path, column=column,batch_size=batch_size)
        return maintable
    else:
...
def write_datasets_to_ms(zarr_path: str, msname: str, column: str, batch_size:int):
    ...
    total_steps = 1 + len(baselines) + 1 + 3 + 1 + subtable_count + 1 + len(baselines)*len(correlations) / batch_size
