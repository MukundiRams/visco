...
CORR_TYPES_REVERSE = OmegaConf.load(
    f"{visco.PCKGDIR}/ms_corr_types_reverse.yaml"
).CORR_TYPES
...
def write_table_to_zarr(
    ...
    for i, ds in enumerate(dataset):
        ...
        encoding = {
            var: {
                "compressor": codec,
                "chunks": (chunk_size_row,) + ds[var].shape[1:]
                if chunk_size_row
                else ds[var].chunks,
            }
...
def write_ms_to_zarr(
    ...
    def list_subtables(ms_path):
        return [
            f for f in os.listdir(ms_path) if os.path.isdir(os.path.join(ms_path, f))
        ]
...
def estimate_flagged_data(maintable: xr.Dataset) -> xr.Dataset:
    ...
    def interpolate_and_replace(vis, flag, weight, block_info=None):
        ...
        interpolated = griddata(
            (u_valid, v_valid), vis_valid, flagged_uv, method="linear", fill_value=0.0
        )
...
def apply_svd(
    visdata: da.Array, decorrelation: float = None, compressionrank: int = None
):
    """
...
def batch_baselines(baselines: list, batch_size: int):
    ...
    return [
        baseline_list[i : i + batch_size]
        for i in range(0, len(baseline_list), batch_size)
    ]


def compress_visdata(
    ...
    if scan not in scans:
        raise ValueError(
            f"Invalid SCAN_NUMBER {scan}. Available scans: {scans.tolist()}"
        )
    ...
    maintable = ds.where(
        (ds.SCAN_NUMBER == scan) & (ds.DATA_DESC_ID == ddid) & (ds.FIELD_ID == fieldid)
    )
    ...
    if "WEIGHT_SPECTRUM" in maintable.data_vars:
        ...
        write_svd_to_zarr(
            wscomps, ws_path, compressor, level, maintable.coords["ROWID"].values
        )
    ...
    if use_model_data:
        ...
        mod_data = (
            maintable.MODEL_DATA.data
            if model_data is None
            else maintable[model_data].data
        )
        maintable_copy[column].data = da.where(
            maintable_copy.FLAG.data, mod_data, maintable_copy[column].data
        )
        ...
        if isinstance(flagvalue, str):
            try:
                flagvalue = complex(flagvalue.replace(" ", ""))
            except ValueError:
                try:
                    flagvalue = float(flagvalue)
                except ValueError:
                    raise ValueError(
                        f"Invalid flagvalue '{flagvalue}'. Use a float or complex format like '1+1j'."
                    )

        log.warning(f"Replacing flagged data with {flagvalue} - may amplify noise")
        maintable_copy[column].data = da.where(
            maintable_copy.FLAG.data, flagvalue, maintable_copy[column].data
        )
    ...
    for batch_num, baseline_batch in enumerate(baseline_batches):
        if _global_progress:
            progress_pct = (
                int((processed_baselines / total_baselines) * 100)
        ...
        for antenna1, antenna2 in baseline_batch:
            ...
            if correlation_optimized:
                ...
                if "XX" in correlation.split(",") and "YY" in correlation.split(","):
                    ...
                    save_task = delayed(write_svd_to_zarr)(
                        task, save_path, compressor, level, diag_row_idx
                    )
                ...
                if "XY" in correlation.split(",") and "YX" in correlation.split(","):
                    ...
                    save_task = delayed(write_svd_to_zarr)(
                        task, save_path, compressor, level, off_diag_row_idx
                    )
                    batch_tasks.append(save_task)
            else:
                ...
                for c in corr_list_user:
                    ...
                    if isinstance(baseline_data[column].data, da.Array):
                        ...
                        visdata = da.from_array(
                            baseline_data[column].data[:, :, ci],
                    ...
                    save_task = delayed(write_svd_to_zarr)(
                        task, save_path, compressor, level, row_idx
                    )
                    batch_tasks.append(save_task)
        ...
        if batch_tasks:
            if _global_progress:
                _global_progress.set_description(
                    f"Computing batch {batch_num + 1}/{len(baseline_batches)}"
                )
...
def write_a_group_to_zarr(zarr_path: str, group: str, data: np.ndarray):
    ...
    data : np.ndarray
        Data to write.
    """
    ds = xr.Dataset({group: (("row"), data)}, coords={"row": np.arange(data.shape[0])})
    ds.to_zarr(zarr_path, group=f"{group}", mode="a")


def write_svd_to_zarr(
    svd_result, path: Path, compressor: str, level: int, rowid: np.ndarray
):
    """
    ...
    ds.to_zarr(
        store,
        mode="a",
        encoding={var: {"compressor": compressor_obj} for var in ds.data_vars},
    )
...
def compress_full_ms(
    ...
    work_breakdown = calculate_total_work(
        ms_path, correlation, correlation_optimized, batch_size, antennas
    )
    total_work = sum(work_breakdown.values())

    _global_progress = tqdm(
        total=total_work, desc="MS Compression Pipeline", unit="tasks"
    )
